        ...
        for segment in plan.segments() {
            ...
            loop {
                if let Some((_, flags)) = self.address_space.translate(page) {
                    ...
                    if writable != segment.writable || executable != segment.executable { return Err(LoadError::IncompatiblePage); }
                } else { self.address_space.map_zeroed_user_page(page, segment.writable, segment.executable)?; }
                if page == last { break; }
            ...
            self.zero_user_memory(segment.virtual_address.checked_add(segment.file_size).ok_or(LoadError::Arithmetic)?, segment.memory_size - segment.file_size)?;
        }
