//! Process ownership and ELF loading for AlohaOS user programs.
...
impl Process {
    pub fn new(pid: u64) -> Result<Self, MapError> {
        ...
        Ok(Self { pid, state: ProcessState::Ready, entry: USER_CODE_BASE, user_stack_top: USER_STACK_TOP, exit_code: 0, address_space, code_frame, kernel_stack_start })
    }
    ...
    pub fn load_elf(&mut self, image: &[u8]) -> Result<(), LoadError> {
        ...
        for segment in plan.segments() {
            let segment_end = segment.virtual_address.checked_add(segment.memory_size).ok_or(LoadError::Arithmetic)?;
            let mut page = segment.virtual_address & !(memory::FRAME_SIZE - 1);
            let last = (segment_end - 1) & !(memory::FRAME_SIZE - 1);
            loop {
                if let Some((_, flags)) = self.address_space.translate(page) {
                    let writable = flags & WRITABLE_FLAG != 0;
                    let executable = flags & NO_EXECUTE_FLAG == 0;
                    if writable != segment.writable || executable != segment.executable {
                        return Err(LoadError::IncompatiblePage);
                    }
                } else {
                    self.address_space.map_zeroed_user_page(page, segment.writable, segment.executable)?;
                }
                if page == last { break; }
                page = page.checked_add(memory::FRAME_SIZE).ok_or(LoadError::Arithmetic)?;
            }

            let file_start = usize::try_from(segment.file_offset).map_err(|_| LoadError::Arithmetic)?;
            let file_size = usize::try_from(segment.file_size).map_err(|_| LoadError::Arithmetic)?;
            let file_end = file_start.checked_add(file_size).ok_or(LoadError::Arithmetic)?;
            self.initialize_user_memory(segment.virtual_address, &image[file_start..file_end])?;
            self.zero_user_memory(
                segment.virtual_address.checked_add(segment.file_size).ok_or(LoadError::Arithmetic)?,
    ...
    fn zero_user_memory(&self, address: u64, length: u64) -> Result<(), LoadError> {
        ...
        while zeroed < length {
            ...
            let (physical, _) = self.address_space.translate(virtual_address).ok_or(LoadError::IncompatiblePage)?;
            let available = memory::FRAME_SIZE - physical % memory::FRAME_SIZE;
            let count = available.min(length - zeroed);
...
impl Drop for Process {
    fn drop(&mut self) {
        unsafe { let _ = memory::deallocate_contiguous(self.kernel_stack_start, KERNEL_STACK_PAGES); }
    }
}
