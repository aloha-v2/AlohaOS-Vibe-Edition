//! PS/2 set-1 keyboard driver with an IRQ-safe SPSC ring buffer.
...
/// Runs in IRQ context. It never allocates or touches the framebuffer.
pub fn interrupt() {
...
pub fn pop_scancode() -> Option<u8> {
    ...
    if read == WRITE_INDEX.load(Ordering::Acquire) {
        return None;
    }
...
pub fn decode(scancode: u8) -> Option<u8> {
    unsafe {
        ...
        if SHIFT && normal.is_ascii_lowercase() { Some(normal.to_ascii_uppercase()) } else { Some(normal) }
    }
}
