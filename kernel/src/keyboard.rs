...
#[derive(Clone, Copy)]
pub enum Key { Character(u8), Enter, Backspace, Up, Down }

pub fn interrupt() {
    let scancode = unsafe { pic::inb(0x60) };
    let write = WRITE_INDEX.load(Ordering::Relaxed);
    let next = (write + 1) % BUFFER_SIZE;
    if next != READ_INDEX.load(Ordering::Acquire) {
        BUFFER[write].store(scancode, Ordering::Relaxed);
        WRITE_INDEX.store(next, Ordering::Release);
    } else {
        DROPPED.fetch_add(1, Ordering::Relaxed);
    }
    unsafe { pic::end_of_interrupt(KEYBOARD_VECTOR) };
}

pub fn pop_scancode() -> Option<u8> {
    let read = READ_INDEX.load(Ordering::Relaxed);
    if read == WRITE_INDEX.load(Ordering::Acquire) { return None; }
    let value = BUFFER[read].load(Ordering::Relaxed);
    READ_INDEX.store((read + 1) % BUFFER_SIZE, Ordering::Release);
    Some(value)
}

pub fn decode(scancode: u8) -> Option<Key> {
    unsafe {
        if scancode == 0xe0 { EXTENDED = true; return None; }
        if EXTENDED {
            EXTENDED = false;
            return match scancode { 0x48 => Some(Key::Up), 0x50 => Some(Key::Down), _ => None };
        }
        match scancode {
            0x2a | 0x36 => { SHIFT = true; return None; }
            0xaa | 0xb6 => { SHIFT = false; return None; }
            code if code & 0x80 != 0 => return None,
            _ => {}
        }
        if scancode == 0x1c { return Some(Key::Enter); }
        if scancode == 0x0e { return Some(Key::Backspace); }

        let normal = match scancode {
            0x02..=0x0a => b'1' + (scancode - 2),
            0x0b => b'0',
            0x0c => b'-',
        ...
        let character = if SHIFT && normal.is_ascii_lowercase() {
