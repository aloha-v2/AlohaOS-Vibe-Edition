...
use crate::{gdt,process::{Process,ProcessState},process_runtime,process_table,syscall_arch,syscall_entry::ReturnPath};
...
pub fn run(process:&mut Process)->u64{LAST_MARKER.store(NO_MARKER,Ordering::Release);process.mark_running();syscall_arch::install_process(process);{let guard=process.address_space.activate();unsafe{aloha_enter_user(process.entry,process.user_stack_top,gdt::user_code_selector()as u64,gdt::user_data_selector()as u64);}drop(guard);}syscall_arch::uninstall_process();let marker=LAST_MARKER.load(Ordering::Acquire);if marker==NO_MARKER&&process.state==ProcessState::Running{process.fault();}else if marker!=NO_MARKER{process.exit(0);}marker}
