//! Two-task preemptive round-robin scheduler.
//!
//! The extended context switch path has passed hardware smoke, 60-second CI
//! stress and a one-hour QEMU stress run. Preemption is therefore enabled at
//! boot; the shell gate remains available as an emergency diagnostic control.
...
pub fn init() {
    ...
    if prepare_worker() {
        ...
        serial::info(format_args!("scheduler: preemption enabled by default"));
        #[cfg(feature = "scheduler-stress")]
        serial::info(format_args!("scheduler-stress: preemption enabled"));
    } else {
        serial::error(format_args!(
            "scheduler: setup failed, continuing without preemption"
        ));
    }
}
...
extern "C" fn worker_task() -> ! {
    loop {
        ...
        #[cfg(feature = "scheduler-stress")]
        if heartbeat == 100 {
            serial::info(format_args!("scheduler-stress: worker heartbeat 100"));
        }
        #[cfg(not(feature = "scheduler-stress"))]
        core::hint::black_box(heartbeat);
...
pub fn sleep_until(task: usize, wake_tick: u64) -> bool {
    let Some(deadline) = WAKE_TICKS.get(task) else {
        return false;
    };
...
pub fn exit(task: usize) -> bool {
    let Some(slot) = STATES.get(task) else {
        return false;
    };
    loop {
