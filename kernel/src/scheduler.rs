...
//!
//! Normal boots leave preemption gated until `sched on`. The
//! `scheduler-stress` feature prepares the worker and opens the same gate at
//! boot so headless QEMU can continuously exercise the real switch path.
...
impl TaskState { fn from_raw(value:u8)->Self { match value {0=>Self::Ready,1=>Self::Running,2=>Self::Blocked,3=>Self::Sleeping,4=>Self::Dead,_=>Self::Dead} } }
#[derive(Clone, Copy)]
...
pub fn init() {
    ...
    #[cfg(feature = "scheduler-stress")]
    {
        if prepare_worker() {
            ...
            serial::info(format_args!("scheduler-stress: preemption enabled"));
        } else {
            serial::error(format_args!("scheduler-stress: setup failed"));
        }
    }
}

fn prepare_worker()->bool {
    if WORKER_PREPARED.load(Ordering::Acquire) { return true; }
    if !task_context::init() { serial::error(format_args!("scheduler: context initialization failed")); return false; }
    let Some(stack_top)=task_stacks::stack_top(1) else { serial::error(format_args!("scheduler: worker stack unavailable")); return false; };
    let prepared=unsafe{task_context::prepare_kernel_task(1,stack_top,worker_task as *const() as u64,task_returned as *const() as u64,KERNEL_CODE_SELECTOR,KERNEL_DATA_SELECTOR)};
    if !prepared { serial::error(format_args!("scheduler: worker frame unavailable")); return false; }
...
extern "C" fn worker_task()->! {
    loop {
        let heartbeat=WORKER_HEARTBEAT.fetch_add(1,Ordering::Relaxed)+1;
        #[cfg(feature = "scheduler-stress")]
        if heartbeat==100 { serial::info(format_args!("scheduler-stress: worker heartbeat 100")); }
        unsafe{asm!("sti","hlt",options(nomem,nostack))};
    }
}
extern "C" fn task_returned()->! { PREEMPTION_ENABLED.store(false,Ordering::Release);let _=exit(CURRENT.load(Ordering::Acquire));loop{unsafe{asm!("cli","hlt",options(nomem,nostack))};} }

pub fn state(task:usize)->Option<TaskState>{STATES.get(task).map(|value|TaskState::from_raw(value.load(Ordering::Acquire)))}
...
pub fn block(task:usize)->bool{if let Some(value)=WAKE_TICKS.get(task){value.store(NO_WAKE_DEADLINE,Ordering::Relaxed);}transition(task,TaskState::Ready,TaskState::Blocked)||transition(task,TaskState::Running,TaskState::Blocked)}
pub fn sleep_until(task:usize,wake_tick:u64)->bool{let Some(deadline)=WAKE_TICKS.get(task)else{return false};let changed=transition(task,TaskState::Ready,TaskState::Sleeping)||transition(task,TaskState::Running,TaskState::Sleeping);if changed{deadline.store(wake_tick,Ordering::Release);}changed}
pub fn exit(task:usize)->bool{let Some(slot)=STATES.get(task)else{return false};loop{let current=TaskState::from_raw(slot.load(Ordering::Acquire));if current==TaskState::Dead{return false}if slot.compare_exchange_weak(current as u8,TaskState::Dead as u8,Ordering::AcqRel,Ordering::Acquire).is_ok(){WAKE_TICKS[task].store(NO_WAKE_DEADLINE,Ordering::Release);return true;}}}
...
pub fn set_preemption(enabled:bool)->bool{if CURRENT.load(Ordering::Acquire)!=0{return false}if enabled{if !prepare_worker()||state(1)!=Some(TaskState::Ready){return false}PREEMPTION_ENABLED.store(true,Ordering::Release);}else{PREEMPTION_ENABLED.store(false,Ordering::Release);}true}
...
pub extern "C" fn scheduler_on_timer_tick(stack:u64,tick:u64)->u64{
    ...
    if !PREEMPTION_ENABLED.load(Ordering::Acquire){return stack}
    let current=CURRENT.load(Ordering::Relaxed);let mut next=current;
    for offset in 1..=TASK_COUNT{let candidate=(current+offset)%TASK_COUNT;if matches!(state(candidate),Some(TaskState::Ready)|Some(TaskState::Running)){next=candidate;break;}}
    if next==current{return stack}
    ...
    CURRENT.store(next,Ordering::Release);SWITCHES[next].fetch_add(1,Ordering::Relaxed);
...
pub fn context_switching_ready()->bool{PREEMPTION_ENABLED.load(Ordering::Acquire)}
