...
#[repr(C)]
struct InterruptFrame { r15:u64,r14:u64,r13:u64,r12:u64,rbp:u64,rbx:u64,r11:u64,r10:u64,r9:u64,r8:u64,rdi:u64,rsi:u64,rdx:u64,rcx:u64,rax:u64,rip:u64,cs:u64,rflags:u64 }

pub fn init(){
 PREEMPTION_TICKS.store(0,Ordering::Relaxed);CURRENT.store(0,Ordering::Relaxed);STATES[0].store(TaskState::Running as u8,Ordering::Release);
 for task in 0..TASK_COUNT{WAKE_TICKS[task].store(NO_WAKE_DEADLINE,Ordering::Relaxed);SWITCHES[task].store(0,Ordering::Relaxed);SAVED_STACKS[task].store(0,Ordering::Relaxed)}
 if task_context::init(){if let Some(stack)=unsafe{build_initial_frame(1,worker_task as usize as u64)}{SAVED_STACKS[1].store(stack,Ordering::Release);STATES[1].store(TaskState::Ready as u8,Ordering::Release);return}}
 STATES[1].store(TaskState::Blocked as u8,Ordering::Release);
}

unsafe fn build_initial_frame(task:usize,entry:u64)->Option<u64>{
 let top=task_stacks::stack_top(task)?;let stack=top.checked_sub(size_of::<InterruptFrame>()as u64)?;let frame=&mut*(stack as *mut InterruptFrame);
 *frame=InterruptFrame{r15:0,r14:0,r13:0,r12:0,rbp:0,rbx:0,r11:0,r10:0,r9:0,r8:0,rdi:0,rsi:0,rdx:0,rcx:0,rax:0,rip:entry,cs:KERNEL_CODE_SELECTOR,rflags:INTERRUPT_FLAGS};Some(stack)
}
...
pub extern "C" fn scheduler_on_timer_tick(current_stack:u64,tick:u64)->u64{
 ...
 let current=CURRENT.load(Ordering::Relaxed);SAVED_STACKS[current].store(current_stack,Ordering::Release);
 ...
 unsafe{task_context::switch(current,next)};CURRENT.store(next,Ordering::Release);SWITCHES[next].fetch_add(1,Ordering::Relaxed);SAVED_STACKS[next].load(Ordering::Acquire)
}
