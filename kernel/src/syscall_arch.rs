...
#[no_mangle]pub extern "C" fn rust_user_fault(vector:u64,error:u64,rip:u64,address:u64)->!{let pointer=active_process_ptr();if pointer.is_null(){crate::serial::emergency(format_args!("user fault without active process"));crate::halt()}let process=unsafe{&mut*pointer};let code=-((vector as i32).max(1));process.fault();let _=process_table::fault(process.pid,code);process_table::orphan_children(process.pid);let return_rsp=unsafe{ptr::read_volatile(&raw const ALOHA_USER_RETURN_RSP)};crate::serial::info(format_args!("user[{}] fault vector={} error={:#x} rip={:#x} address={:#x} runner_rsp={:#x}",process.pid,vector,error,rip,address,return_rsp));if return_rsp==0{crate::halt()}unsafe{aloha_return_process_runner(return_rsp)}}
