...
#[no_mangle]pub extern "C" fn rust_user_fault(vector:u64,error:u64,rip:u64,address:u64)->!{let pointer=active_process_ptr();if pointer.is_null(){serial::emergency(format_args!("USER_FAULT no process vector={} rip={:#x}",vector,rip));crate::halt()}let process=unsafe{&mut*pointer};let code=-((vector as i32).max(1));let direct=unsafe{ptr::read_volatile(&raw const ALOHA_USER_RETURN_RSP)};let kernel=unsafe{ptr::read_volatile(&raw const ALOHA_SYSCALL_KERNEL_RETURN_RSP)};serial::info(format_args!("user[{}] fault vector={} error={:#x} rip={:#x} address={:#x} pid={} state={:?} direct_rsp={:#x} kernel_rsp={:#x}",process.pid,vector,error,rip,address,process.pid,process.state,direct,kernel));process.fault();let table_result=process_table::fault(process.pid,code);let orphaned=process_table::orphan_children(process.pid);serial::info(format_args!("USER_FAULT handled table={:?} orphaned={} state={:?}",table_result,orphaned,process.state));if direct==0{serial::emergency(format_args!("USER_FAULT no direct runner rsp, halting"));crate::halt()}unsafe{aloha_return_process_runner(direct)}}
