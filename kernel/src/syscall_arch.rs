...
unsafe extern "C"{fn aloha_syscall_entry();static mut ALOHA_SYSCALL_KERNEL_STACK:u64;static mut ALOHA_SYSCALL_KERNEL_RETURN_RSP:u64;static mut ALOHA_SYSCALL_PROCESS:u64;}
...
/// Transfer exclusive process access to the single-core syscall trampoline.
...
#[no_mangle]pub extern "C" fn rust_syscall_dispatch(frame:*mut SyscallFrame)->u8{let process_ptr=unsafe{ptr::read_volatile(&raw const ALOHA_SYSCALL_PROCESS)};if frame.is_null()||process_ptr==0{return 1}let process=unsafe{&mut*(process_ptr as*mut Process)};let frame=unsafe{&mut*frame};let terminated=dispatch_frame(process,frame);if terminated{return 1}let candidate=UserReturnFrame{rip:frame.user_rip,rsp:frame.user_rsp,rflags:frame.user_rflags};match candidate.return_path(){ReturnPath::Sysret=>{frame.user_rflags=candidate.sanitized().rflags;0}ReturnPath::Iret|ReturnPath::Reject=>{process.fault();1}}}
...
#[inline]unsafe fn rdmsr(msr:u32)->u64{let low:u32;let high:u32;asm!("rdmsr",in("ecx")msr,out("eax")low,out("edx")high,options(nostack));(high as u64)<<32|low as u64}
#[inline]unsafe fn wrmsr(msr:u32,value:u64){asm!("wrmsr",in("ecx")msr,in("eax")value as u32,in("edx")(value>>32)as u32,options(nostack));}
