//! IDT, CPU exceptions, hardware IRQs and user fault isolation.

use core::arch::{asm,global_asm};use core::cell::UnsafeCell;use core::mem::size_of;use core::sync::atomic::{AtomicBool,Ordering};use crate::{framebuffer,gdt,halt,keyboard,pic,serial,timer};
pub const USER_TRAP_VECTOR:usize=0x80;
#[derive(Clone,Copy)]#[repr(C,packed)]struct IdtEntry{offset_low:u16,selector:u16,ist:u8,attributes:u8,offset_middle:u16,offset_high:u32,reserved:u32}impl IdtEntry{const MISSING:Self=Self{offset_low:0,selector:0,ist:0,attributes:0,offset_middle:0,offset_high:0,reserved:0};fn gate(a:u64,ist:u8,attributes:u8)->Self{Self{offset_low:a as u16,selector:gdt::code_selector(),ist:ist&7,attributes,offset_middle:(a>>16)as u16,offset_high:(a>>32)as u32,reserved:0}}fn kernel(a:u64,ist:u8)->Self{Self::gate(a,ist,0x8e)}fn user_trap(a:u64)->Self{Self::gate(a,0,0xee)}}
#[repr(C,packed)]struct IdtPointer{limit:u16,base:u64}struct IdtStorage(UnsafeCell<[IdtEntry;256]>);unsafe impl Sync for IdtStorage{}static IDT:IdtStorage=IdtStorage(UnsafeCell::new([IdtEntry::MISSING;256]));static INITIALIZED:AtomicBool=AtomicBool::new(false);
...
isr_invalid_opcode:
 ...
 jnz user_invalid_opcode
 mov rdx,[rsp];xor esi,esi;xor ecx,ecx;mov edi,6;jmp exception_trampoline
user_invalid_opcode:
 mov rdx,[rsp];xor esi,esi;xor ecx,ecx;mov edi,6;jmp user_fault_trampoline
...
isr_page_fault:
 ...
 jnz user_page_fault
 mov rsi,[rsp];mov rdx,[rsp+8];mov rcx,cr2;mov edi,14;jmp exception_trampoline
user_page_fault:
...
.global user_trap_80
user_trap_80:
 cli;mov rdi,rax;and rsp,-16;call rust_user_trap
 mov ax,0x10;mov ds,ax;mov es,ax;mov ss,ax;mov rsp,[rip+ALOHA_USER_RETURN_RSP];ret
"#);