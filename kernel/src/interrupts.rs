//! IDT, CPU exception stubs, and hardware IRQ entry points.

use core::arch::{asm, global_asm};
use core::mem::size_of;
use core::ptr::addr_of;
use crate::{framebuffer, gdt, halt, keyboard, pic};

#[derive(Clone, Copy)]
#[repr(C, packed)]
struct IdtEntry {
    offset_low: u16, selector: u16, ist: u8, attributes: u8,
    offset_middle: u16, offset_high: u32, reserved: u32,
}
impl IdtEntry {
    ...
    fn handler(address: u64, ist: u8) -> Self {
        Self { offset_low: address as u16, selector: gdt::code_selector(), ist: ist & 7,
            attributes: 0x8e, offset_middle:(address >> 16) as u16,
            offset_high:(address >> 32) as u32, reserved:0 }
    }
}
#[repr(C, packed)]
...
unsafe extern "C" {
    fn isr_divide_error(); fn isr_breakpoint(); fn isr_invalid_opcode();
    fn isr_double_fault(); fn isr_general_protection(); fn isr_page_fault();
    fn irq_keyboard();
}
...
pub fn init() {
    unsafe {
        ...
        let pointer = IdtPointer { limit:(size_of::<[IdtEntry;256]>() - 1) as u16, base:addr_of!(IDT) as u64 };
...
#[no_mangle]
pub extern "C" fn rust_keyboard_interrupt() { keyboard::interrupt(); }

#[no_mangle]
pub extern "C" fn rust_exception_handler(vector:u64, error_code:u64, instruction_pointer:u64, fault_address:u64) -> ! {
    unsafe { asm!("cli", options(nomem, nostack)) };
    let name = match vector { 0=>"DIVIDE BY ZERO",3=>"BREAKPOINT",6=>"INVALID OPCODE",8=>"DOUBLE FAULT",13=>"GENERAL PROTECTION FAULT",14=>"PAGE FAULT",_=>"UNKNOWN CPU EXCEPTION" };
    framebuffer::panic_header(name);
    framebuffer::write_label_hex("VECTOR: ",vector); framebuffer::write_label_hex("ERROR:  ",error_code);
