...
const MASTER_COMMAND:u16=0x20; const MASTER_DATA:u16=0x21;
const SLAVE_COMMAND:u16=0xa0; const SLAVE_DATA:u16=0xa1; const EOI:u8=0x20;

pub unsafe fn init_timer_and_keyboard(){
    outb(MASTER_COMMAND,0x11);io_wait();outb(SLAVE_COMMAND,0x11);io_wait();
    outb(MASTER_DATA,MASTER_OFFSET);io_wait();outb(SLAVE_DATA,MASTER_OFFSET+8);io_wait();
    outb(MASTER_DATA,4);io_wait();outb(SLAVE_DATA,2);io_wait();
    outb(MASTER_DATA,1);io_wait();outb(SLAVE_DATA,1);io_wait();
    outb(MASTER_DATA,0xfc);outb(SLAVE_DATA,0xff);
}
...
pub fn reboot() -> ! {
    unsafe {
        for _ in 0..100_000 { if inb(0x64)&2==0 { outb(0x64,0xfe); break; } }
        for _ in 0..1_000_000 { asm!("pause",options(nomem,nostack)); }
        // QEMU/modern chipset reset fallback.
        outb(0xcf9,0x06);
